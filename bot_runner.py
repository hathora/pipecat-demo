...
async def spawn_hathora_process(room_url: str, token: str):

    hathora_endpoint = os.getenv("HATHORA_CLOUD_ENDPOINT", "https://api.hathora.io")
    hathora_app_id = os.getenv("HATHORA_APP_ID_BOTS")
    hathora_api_key = os.getenv("HATHORA_TOKEN")

    hathora_headers = {"Authorization": f"Bearer {hathora_api_key}", "Content-Type": "application/json"}

    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{hathora_endpoint}/rooms/v2/{hathora_app_id}/create", headers=hathora_headers,
            json={
                "region": "Washington_DC",
                "roomConfig": json.dumps({
                    "room_url": room_url,
                    "token": token,
                })
            }
            ...
            if r.status != 201:
                text = await r.text()
                raise Exception(f"Unable to create Hathora room {r.status}: {text}")

            res = await r.json()
            print(f"Hathora room created. Room ID: {res['roomId']}, Process ID: {res['processId']}")
